...
class BreachArchive {
    ...
    async init() {
        ...
        this.updateStats();
        await this.updateFrontPageStats(); // Await this call as it will fetch file modification time
    }
    ...
    async loadBreaches() {
        try {
            ...
            if (response.ok) {
                ...
                console.log('Loaded breaches from data/breaches.json:', this.breaches.length, 'breaches');
            } else {
                // Fallback to sample data
                console.log('Failed to load breaches.json, using sample data');
            ...
            console.log('Error loading breaches.json, using sample data:', error);
            this.breaches = this.getSampleBreaches();
            this.filteredBreaches = [...this.breaches];
            this.renderBreaches();
            const loadingElement = document.getElementById('loading');
        ...
        modal.innerHTML = `
            <div class="glass rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-6 mb-6">
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-2xl font-bold gradient-text">${this.formatNumber(breach.records)}</p>
                            </div>
                            <div>
                                <h4 class="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-1">Severity Level</h4>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <h4 class="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-1">Archive Password</h4>
                                <p class="text-green-400 font-mono">${breach.password}</p>
                            </div>
                            <div>
                                <h4 class="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-1">Industry</h4>
                                <p class="text-white">${breach.type}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        <h4 class="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-2">Tags</h4>
                    ...
                    <div class="flex gap-4">
                        ...
                        <button onclick="this.parentElement.parentElement.parentElement.parentElement.remove()" 
                                class="px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg font-semibold transition-colors duration-200">
                            Close
                        </button>
                    </div>
                </div>
    ...
    downloadBreach(url) {
        // Show notification
        this.showNotification('Opening download link...', 'info');
    ...
    // Update stats for the statistics page
    updateStats() {
        ...
        console.log('Updating stats page with:', {
            totalBreaches,
            totalRecords,
            totalCompanies,
            highSeverity,
        ...
        // Animate counters for stats page
    ...
    // Update stats for the front page stats bar
    async updateFrontPageStats() {
        const totalBreaches = this.breaches.length;
        const totalRecords = this.breaches.reduce((sum, breach) => sum + breach.records, 0);
        const totalCompanies = new Set(this.breaches.map(breach => breach.company)).size;
        ...
        let lastUpdated = 'N/A';
        try {
            const response = await fetch('./data/breaches.json', { method: 'HEAD' });
            if (response.ok) {
                const lastModified = response.headers.get('Last-Modified');
                if (lastModified) {
                    lastUpdated = this.formatDateShort(new Date(lastModified));
                }
            }
        } catch (error) {
            console.error('Error fetching Last-Modified header for breaches.json:', error);
        }
        
        console.log('Updating front page stats with:', {
            totalBreaches,
            totalRecords,
            totalCompanies,
            lastUpdated
        ...
        // Animate counters for front page
        ...
        const lastUpdatedElement = document.getElementById("latest-update");
