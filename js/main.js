...
class BreachArchive {
    ...
    updateFrontPageStats() {
        ...
        // Get the most recent breach date for "Last Updated"
        const mostRecentDate = this.breaches.reduce((latest, breach) => {
            const breachDate = new Date(breach.date);
            return breachDate > latest ? breachDate : latest;
        }, new Date(0));
        
        const lastUpdated = this.formatDateShort(mostRecentDate);
